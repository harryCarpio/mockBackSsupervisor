...
class IncidentRequest(BaseModel):
    descripcion: str = "Pluma no se eleva al ingresar."
...
async def login(request: LoginRequest):
    ...
    refresh_token = create_access_token({"sub": request.username, "type": "refresh"}, timedelta(days=7))
...
@app.get("/api/v1/tickets/calculate", response_model=TicketCalculateResponse, tags=["Tickets"])
...
@app.post("/api/v1/payments/cash", response_model=PaymentCashResponse, tags=["Payments"])
async def payment_cash(
    request: PaymentCashRequest, 
    token: dict = Depends(verify_token)
):
    """
...
@app.get("/api/v1/payments/verify", response_model=PaymentVerifyResponse, tags=["Payments"])
